...
def process_game():
    ...
    while game is not None:
        if game.headers["WhiteElo"] == "?" or game.headers["BlackElo"] == "?":
            a += 1
            if a % 1000 == 0:
                print(f"Finished game #{a} out of 1,209,291 ({float(a) / 1209291 * 100}%)")
            game = chess.pgn.read_game(pgn_game)
            continue

        game_rating_diff = int(game.headers["WhiteElo"]) - int(game.headers["BlackElo"])
        index = 0 if game_rating_diff >= 0 else 1
...
def plot_upset_chances():
    ...
    plt.title("Chances of Winning if the Player has more ELO")
